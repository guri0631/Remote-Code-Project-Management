...
// Author        : Arjun Kalyanasundram, Syracuse University             //
...
{
  std::vector<std::string> s1 = cconv_.convert(files_);
  //std::vector<std::string> s1 = cconv_.convert();
    //s1 = CodePublisher::publish(files_);
  return s1;
}
...
// -----<function to return files as a vector >-----------------------------------------------------
std::vector<std::string> CodePublisher::Conv_send(int argc, char **argv)  
{
  CodePublisher cp;
  checkn(cp, argc, argv);        
  std::vector<std::string> publish_vec;  
  publish_vec = demoReq67(cp);
  return publish_vec;          

}
int CodePublisher::checkn(CodePublisher& cp, int argc, char ** argv)
{
	int check = demoReq45(cp, argc, argv);
	return check;
}

int CodePublisher::demoReq45(CodePublisher& cp, int argc, char ** argv) 
{
...
std::vector<std::string> CodePublisher::demoReq67(CodePublisher& cp) 
{

  std::vector<std::string> s= cp.publish();
  return s;
