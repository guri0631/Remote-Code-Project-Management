...
bool Sender::sendFile(Message msg)
{
  ...
  //std::iostream sendFile(fileSpec);
  std::ifstream sendFile(fileSpec);
  if (!sendFile.good())
    return false;
  while (true)
  {
...
public:
  ...
  ClientHandler(BlockingQueue<Message>* pQ, const std::string& rcvPath,
    const std::string& name = "clientHandler") : pQ_(pQ),
    saveFilepath(rcvPath),
    clientHandlerName(name)
  {
